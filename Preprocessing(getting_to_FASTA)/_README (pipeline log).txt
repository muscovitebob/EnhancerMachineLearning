...
------- : 
o	awk -F"," '{ print $3 "\t" $4  "\t" $5 }'  bed_withrsadded.csv |uniq | tr -d ' ' | tr -d '\r'| sed '1d' > complete.bed
o	remark :  the original file contained SNV of the enhancers, and also both variants of the SNP.  This will create double ranges. These variants and minor alleles are filtered out, so are not considered in the final file.   (these could be obtained by matching with a second supplementary file GSE75661_7.5k_Barcode_Oligo.tab.gz, but at this point this is not done)
