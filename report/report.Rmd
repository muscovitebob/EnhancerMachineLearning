...
Accuracy is 0.531 can be seen from the accuracy crosstab, Fig. [@fig:accuracycrosstab]. It is calculated as the total amount of correctly predicted possitives and negatives divided by the total amount of predicted sequences. It is relatively low although it shows the RF model is doing better than random choice (0.5).

![Accuracy ](pics/ROC_RF.jpg){#fig:accuracycrosstab}

The ROC curve and AUC evalutation have better results than accuracy,the resulting AUC is 0.67 as can be seen in Fig. [@fig:RFroc1].
...
Both models are tested on a balanced test set of 45333 sequences (20% of the total). From Table [@tab:comparingclassperf] and Figure [@fig:comparingclassperf]  it is clear that model one outperforms model two. The model using multiple convolutional layers achieves an accuracy that is 9% higher then the model using the recurrent layer.

![Comparing classification performance of both models](pics/table1_deeplearning_performance.JPG){#tab:comparingclassperf}

![ROC and Precision Recall curves comparing both models](pics/Fig3DeepLearing_ROCandPR.JPG){#fig:comparingclassperf}

### Motifs detected

The motifs detected by both models are shown in Fig. [@fig:deeplearnmotifmatches].
