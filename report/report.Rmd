...
Boruta is an all relevant feature selection method, meaning it aims at finding relevant features that are suitable for the prediction of the outcome. [@Homola] It seems to be similar to MAST, however, it has a different test mechanism to discover relevant features. Its algorithm involves training the model in each of its iteration. 

ZIXUAN CAN YOU ADD SOME USEFUL STUF HERE? SOMETHING ABOUT THE PARTICULAR METHOD YOU USED? 

### Construction of the model

For the construction of the random forests ensemble model, the python package \texttt{scikit-learn} was used, more specifically, its class \texttt{sklearn.ensemble.RandomForestClassifier}, which offers a standard RF classifier. The classifier was constructed using following parameters: \texttt{n\_jobs=-1}, \texttt{n\_estimators=1000}, \texttt{max\_depth=None}, \texttt{min\_samples\_split=2}, \texttt{class\_weight='balanced'}

BORIS MAYBE HERE YOU COULD ADD SOME EXPLANATION ABOUT THESE PARAMETERS?
...
![Obtained ROC curve for the RF model](pics/ROC_RF.jpg)
