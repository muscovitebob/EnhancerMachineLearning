...
Boruta is an all-relevant feature selection method [@Homola]. Boruta perfroms a top-down search for relative features, making use of an iterative algorithm. In each iteration, a random forests classifier is trained on two data sets: the original data and a shuffled set. In each of these iterations, the most predictive features are captured. It involves a wrapper of the random forests classifier algorithm, in an attempt to capture the important and relevant features for that specific classfier, rather than selecting significant features, like a log likelihood significance test would do. [@Pathak]
