...
MAST reduced the 333 features in original training set to 164 significant features. Then Boruta takes the reduced feature matrix as input and selected 19 important features, which their names can be seen in fig ???. The 'Homer' postfix in some features name indicates they are de novo discovered motifs.

![19 features selected by Boruta algorithm](pics/RF19selected.JPG )
...
Accuracy is 0.531 can be seen from the accuracy crosstab, fig ???. It is calculated as the total amount of correctly predicted possitives and negatives divided by the total amount of predicted sequences. It is relatively low although it shows the RF model is doing better than random choice (0.5).

![Accuracy ](pics/ROC\_RF.jpg)

The ROC curve and AUC evalutation have better results than accuracy,the resulting AUC is 0.67 as can be seen in fig ???.

![Obtained ROC curve for the RF model](pics/ROC4\_balanced.png )

### Motif logos

The logos of the selected motifs are shown in fig ???.

## Deep learning

### Comparing classification performance

From Figure? and Figure??  it is clear that model 1 outperforms model 2. The model using multiple convolutional layers achieves an accuracy that is 9% higher then the model using the recurrent layer.
...
The motifs detected by both models are shown in fig ??
